...
public class RayTracingUtils {
  ...
  public static Vec3 adjustLook(
    ...
    ChatUtils.chat("Found " + points.size());
  ...
  public static MovingObjectPosition rayTraceBlocks(
    ...
    if (!(Double.isNaN(vec31.xCoord) || Double.isNaN(vec31.yCoord) || Double.isNaN(vec31.zCoord))) {
      if (!(Double.isNaN(vec32.xCoord) || Double.isNaN(vec32.yCoord) || Double.isNaN(vec32.zCoord))) {
        ...
        IBlockState iblockstate = Dependencies.mc.theWorld.getBlockState(blockpos);
        ...
        if (
          ...
          (
            ...
            block.getCollisionBoundingBox(Dependencies.mc.theWorld, blockpos, iblockstate) != null
          ) &&
        ...
        while (k1-- >= 0) {
          ...
          IBlockState iblockstate1 = Dependencies.mc.theWorld.getBlockState(blockpos);
          Block block1 = iblockstate1.getBlock();
          if (visualize) blockPosConcurrentLinkedQueue.add(blockpos);
          if (
            ignoreBlockWithoutBoundingBox &&
            block1.getCollisionBoundingBox(Dependencies.mc.theWorld, blockpos, iblockstate1) == null
          ) continue;
  ...
  private static IBlockState getBlockState(BlockPos blockPos) {
    if (Dependencies.mc.theWorld == null) return null;
  ...
  private static boolean canCheck(BlockPos block, Vec3 routeBlock1, BlockPos routeBlock2) {
    if (
      (
        MathUtils.distanceFromTo(block, BlockUtils.fromVecToBP(routeBlock1)) < 4.61 ||
        MathUtils.distanceFromTo(block, routeBlock2) < 4.61
      ) &&
      (
        Dependencies.mc.theWorld.getBlockState(block).getBlock() == Blocks.stained_glass ||
        Dependencies.mc.theWorld.getBlockState(block).getBlock() == Blocks.stained_glass_pane
      )
    ) {
      return true;
    }

    ChatUtils.chat(String.valueOf(MathUtils.distanceFromTo(block, BlockUtils.fromVecToBP(routeBlock1))));
